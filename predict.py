...
def predict(unsup=True, path="inference",model="flownet"):
    file_names = sorted(os.listdir(path))

    if unsup:
        mymodel = Unsupervised(conv_predictor=model)
        modelname = os.path.join("Unsupervised", type(mymodel.predictor).__name__)
    else:
        if "light" in model:
        ...
        modelname = type(mymodel).__name__

    os.makedirs(os.path.join("result", modelname), exist_ok=True)
    mymodel.load_state_dict(torch.load(os.path.join("model_weight", modelname, 'best_weight.pt'), map_location=device)['model_state_dict'])
    if unsup:
    ...
    for i in range(0, len(file_names) - 1, 2):
        frame1 = cv2.imread(os.path.join(path, file_names[i]))
        h, w = frame1.shape[:2]
        frame2 = cv2.imread(os.path.join(path, file_names[i + 1]))
        ...
        pred_flo = computeImg(pred_flo.cpu().numpy(), verbose=True, savePath=os.path.join("result", modelname,
                                                                                         'predicted_flow' + str(
                                                                                             i//2 + 1) + '.png'))
