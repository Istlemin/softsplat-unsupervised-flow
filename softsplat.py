...
def softsplat(
    ...
    # print("teIn:",tenIn[3,:,2,6].tolist())
    # print("tenOut:",tenOut[3,:,2,5].tolist())
    ...
    if strMode.split("-")[0] in ["avg", "linear", "soft"]:
        ...
        if strMode.split("-")[0] == "avg":
            tenNormalize = tenNormalize.clip(0.000001, None)
        else:
            tenNormalize = tenNormalize.clip(0.000001, None)
        # end
...
def pytorch_splat(images,flows):
    n,c,h,w = images.shape
    assert flows.shape == (n,2,h,w)

    y = torch.arange(h,device=images.device).reshape((h,1)).broadcast_to((c,h,w))
    x = torch.arange(w,device=images.device).reshape((1,w)).broadcast_to((c,h,w))
    to_x = x.reshape((1,c,h,w)) + flows[:,:1,:,:]
    to_y = y.reshape((1,c,h,w)) + flows[:,1:,:,:]

    intNorthwestX = (to_x).floor().int()
    intNorthwestY = (to_y).floor().int()
    ...
    addNorthwest = ((intSoutheastX) - to_x) * ((intSoutheastY) - to_y) * images
    addNortheast = (to_x - (intSouthwestX)) * ((intSouthwestY) - to_y) * images
    addSouthwest = ((intNortheastX) - to_x) * (to_y - (intNortheastY)) * images
    addSoutheast = (to_x - (intNorthwestX)) * (to_y - (intNorthwestY)) * images

    addVals = torch.stack([addNorthwest,addNortheast,addSouthwest,addSoutheast],dim=-1)
    addX = torch.stack([intNorthwestX,intNortheastX,intSouthwestX,intSoutheastX],dim=-1)
    addY = torch.stack([intNorthwestY,intNortheastY,intSouthwestY,intSoutheastY],dim=-1)
    batch = torch.arange(n,device=images.device).reshape((n,1,1,1,1)).broadcast_to((n,c,h,w,4))
    channel = torch.arange(c,device=images.device).reshape((1,c,1,1,1)).broadcast_to((n,c,h,w,4))
    
    batch = batch.flatten()
    channel = channel.flatten()
    addY = addY.flatten()
    addX = addX.flatten()
    addVals = addVals.flatten()
    
    ind = ((batch * c + channel) * h + addY)*w + addX
    val_coord = (addX >= 0) & (addX < w) & (addY >= 0) & (addY < h)

    res = torch.zeros_like(images.flatten())
    res.index_add_(0,ind[val_coord], addVals[val_coord])
    
    return res.reshape(images.shape)